...
moduleFor('run.later', class extends AbstractTestCase {
  ...
  ['@test inception calls to later should run callbacks in separate run loops'](assert) {
    ...
    run(() => {
      ...
      assert.ok(runLoop);
      later(() => {
        assert.ok(getCurrentRunLoop() && getCurrentRunLoop() !== runLoop, 'first later callback has own run loop');
        runLoop = getCurrentRunLoop();
        later(() => {
          assert.ok(getCurrentRunLoop() && getCurrentRunLoop() !== runLoop, 'second later callback has own run loop');
          finished = true;
        }, 40);
      }, 40);
    });
