...
moduleFor('ApplicationInstance', class extends TestCase {
  ...
  ['@test an application instance can be created based upon an application'](assert) {
    appInstance = run(() => ApplicationInstance.create({
      application
    }));
  ...
  ['@test can build and boot a registered engine'](assert) {
    ...
    return chatEngineInstance.boot().then(() => {
      ...
      let singletons = ['router:main', P`-bucket-cache:main`, '-view-registry:main', '-environment:main', 'service:-document', 'event_dispatcher:main'];
