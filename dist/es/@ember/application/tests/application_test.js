...
import { moduleFor, ApplicationTestCase, AbstractTestCase, AutobootApplicationTestCase, DefaultResolverApplicationTestCase, verifyInjection, verifyRegistration, runTask } from 'internal-test-helpers';
...
moduleFor('Application, autobooting multiple apps', class extends ApplicationTestCase {
  ...
  [`@test you can make a new application in a non-overlapping element`](assert) {
    let app = runTask(() => this.createSecondApplication({
      rootElement: '#two'
    }));
    runTask(() => app.destroy());
...
moduleFor('Application', class extends ApplicationTestCase {
  [`@test builds a registry`](assert) {
    ...
    verifyInjection(assert, application, 'service:-routing', 'router', 'router:main'); // DEBUGGING
...
moduleFor('Application, autobooting', class extends AutobootApplicationTestCase {
  ...
  [`@test initialize application via initialize call`](assert) {
    runTask(() => this.createApplication()); // This is not a public way to access the container; we just
