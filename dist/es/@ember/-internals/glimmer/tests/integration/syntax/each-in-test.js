...
import { TogglingSyntaxConditionalsTest, TruthyGenerator, FalsyGenerator } from '../../utils/shared-conditional-tests';
...
applyMixins(BasicEachInTest, new TruthyGenerator([{
  foo: 1
}, EmberObject.create({
  'Not Empty': 1
}), [1], NonEmptyFunction, NonEmptyConstructor]), new FalsyGenerator([null, undefined, false, '', 0, [], EmptyFunction, EmptyConstructor, {}, Object.create(null), Object.create({}), Object.create({
  'Not Empty': 1
}), EmberObject.create()]));
applyMixins(EachInProxyTest, new TruthyGenerator([ObjectProxy.create({
  content: {
    'Not empty': 1
  }
})]), new FalsyGenerator([ObjectProxy.create(), ObjectProxy.create({
  content: null
}), ObjectProxy.create({
  content: {}
}), ObjectProxy.create({
...
moduleFor('Syntax test: {{#each-in}}', BasicSyntaxTest); // Rendering tests
...
moduleFor('Syntax test: {{#each-in}} with POJOs', class extends EachInTest {
  constructor() {
    super(...arguments);
...
moduleFor('Syntax test: {{#each-in}} with EmberObjects', class extends EachInTest {
  constructor() {
    super(...arguments);
    this.allowsSetProp = true;
  }
...
moduleFor('Syntax test: {{#each-in}} with object proxies', class extends EachInTest {
  constructor() {
    super(...arguments);
    this.allowsSetProp = true;
  }
  ...
  ['@test it iterates over the content, not the proxy']() {
    ...
    `);
    runTask(() => set(this.context, 'categories', ObjectProxy.create({
      content: {
        Smartphones: 8203,
        'JavaScript Frameworks': Infinity
      }
...
moduleFor('Syntax test: {{#each-in}} with ES6 Maps', class extends EachInTest {
  createHash(pojo) {
    let map = new Map();
...
if (HAS_NATIVE_SYMBOL) {
  moduleFor('Syntax test: {{#each-in}} with custom iterables', class extends EachInTest {
    createHash(pojo) {
