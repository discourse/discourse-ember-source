...
import { TogglingSyntaxConditionalsTest, TruthyGenerator, FalsyGenerator, ArrayTestCases } from '../../utils/shared-conditional-tests';
...
const TRUTHY_CASES = [['hello'], emberA(['hello']), makeSet(['hello']), new ForEachable(['hello']), ArrayProxy.create({
  content: ['hello']
}), ArrayProxy.create({
  content: emberA(['hello'])
})];
const FALSY_CASES = [null, undefined, false, '', 0, [], emberA([]), makeSet([]), new ForEachable([]), ArrayProxy.create({
  content: []
}), ArrayProxy.create({
  content: emberA([])
})];
...
applyMixins(BasicEachTest, new TruthyGenerator(TRUTHY_CASES), new FalsyGenerator(FALSY_CASES), ArrayTestCases);
...
applyMixins(EachEdgeCasesTest, new FalsyGenerator([true, 'hello', 1, Object, function () {}, {}, {
  foo: 'bar'
...
class EachTest extends AbstractEachTest {
  ...
  ['@test re-using the same variable with different {{#each}} blocks does not override each other']() {
    ...
    runTask(() => {
      set(this.context, 'admins', this.createList([{
        name: 'Tom Dale'
      }]).list);
      set(this.context, 'users', this.createList([{
        name: 'Yehuda Katz'
      }]).list);
    });
    this.assertText('Admin: [Tom Dale] User: [Yehuda Katz]');
  ...
  [`@test an outer {{#each}}'s scoped variable does not clobber an inner {{#each}}'s property if they share the same name - Issue #1315`]() {
    ...
    runTask(() => {
      ...
      set(this.context, 'options', this.createList([{
        label: 'One',
        value: 1
      }, {
        label: 'Two',
  ...
  ['@test it should support {{#each name as |foo|}}, then {{#each foo as |bar|}}']() {
    ...
    runTask(() => set(this.context, 'name', this.createList([this.createList(['caterpillar']).list]).list));
    this.assertText('caterpillar');
  }
...
moduleFor('Syntax test: {{#each}} with array proxies, modifying itself', class extends EachTest {
  createList(items) {
    let proxty = ArrayProxy.create({
      content: emberA(items)
    });
    return {
      list: proxty,
      delegate: proxty
...
moduleFor('Syntax test: {{#each as}} undefined path', class extends RenderingTestCase {
...
moduleFor('Syntax test: {{#each}} with sparse arrays', class extends RenderingTestCase {
...
if (typeof MutationObserver === 'function') {
  moduleFor('Syntax test: {{#each as}} DOM mutation test', class extends RenderingTestCase {
    constructor() {
      super(...arguments);
      this.observer = null;
    }
