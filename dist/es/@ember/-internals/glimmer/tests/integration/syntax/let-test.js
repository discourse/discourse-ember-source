...
moduleFor('Syntax test: {{#let as}}', class extends RenderingTestCase {
  templateFor({
    cond,
    truthy,
    falsy
...
moduleFor('Syntax test: Multiple {{#let as}} helpers', class extends RenderingTestCase {
  ['@test re-using the same variable with different {{#let}} blocks does not override each other']() {
    ...
    runTask(() => {
      set(this.context, 'admin', {
        name: 'Tom Dale'
      });
      set(this.context, 'user', {
        name: 'Yehuda Katz'
      });
    });
    this.assertText('Admin: Tom Dale User: Yehuda Katz');
  ...
  ['@test nested {{#let}} blocks should have access to root context']() {
    ...
    runTask(() => set(this.context, 'committer1', {
      name: 'krisselden'
    }));
