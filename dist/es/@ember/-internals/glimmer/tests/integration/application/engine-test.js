...
moduleFor('Application test: engine rendering', class extends ApplicationTestCase {
  get routerOptions() {
    return {
      location: 'none',
  ...
  ['@test loading substate route works for a child route of an Engine'](assert) {
    ...
    return this.visit('/blog/post/comments').then(() => {
      ...
      return transition.then(() => runTaskNext()).then(() => this.assertText('ApplicationEngineLikes'));
    });
  }
  ...
  ["@test query params don't have stickiness by default between model"](assert) {
    ...
    return this.visit('/blog/category/1?type=news').then(() => {
      ...
      let href = this.element.querySelector('a').href; // check if link ends with the suffix
