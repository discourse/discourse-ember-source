...
moduleFor('Components test: contextual components', class extends RenderingTestCase {
  ...
  ['@test GH#17121 implicit component invocations should not perform string lookup']() {
    ...
    expectAssertion(() => this.render(strip`
          {{#let 'foo-bar' as |foo|}}
            {{foo 1 2 3}}
          {{/let}}
...
class ContextualComponentMutableParamsTest extends RenderingTestCase {
  render(templateStr, context = {}) {
    super.render(`${templateStr}<span class="value">{{model.val2}}</span>`, assign(context, {
      model: {
        val2: 8
      }
    }));
...
applyMixins(ContextualComponentMutableParamsTest, new MutableParamTestGenerator([{
  title: 'param',
