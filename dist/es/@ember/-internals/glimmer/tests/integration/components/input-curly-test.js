...
class InputRenderingTest extends RenderingTestCase {
  ...
  assertTriggersNativeDOMEvents(type) {
    ...
    let actionAttrs = Object.keys(events).map(evt => `${events[evt]}=(action 'run_${evt}')`).join(' ');
...
moduleFor('Components test: {{input}}', class extends InputRenderingTest {
  ...
  ['@test dynamic attributes']() {
    ...
    runTask(() => {
      ...
      set(this.context, 'minlength', 6); // set(this.context, 'size', 21); //NOTE: failing in IE (TEST_SUITE=sauce)
    ...
    runTask(() => {
      ...
      set(this.context, 'minlength', 5); // set(this.context, 'size', 20); //NOTE: failing in IE (TEST_SUITE=sauce)
  ...
  ['@test cursor selection range']() {
    ...
    let input = this.$input()[0]; // See https://ember-twiddle.com/33e506329f8176ae874422644d4cc08c?openFiles=components.input-component.js%2Ctemplates.components.input-component.hbs
    // this.assertSelectionRange(8, 8); //NOTE: this is (0, 0) on Firefox (TEST_SUITE=sauce)

    runTask(() => this.rerender()); // this.assertSelectionRange(8, 8); //NOTE: this is (0, 0) on Firefox (TEST_SUITE=sauce)
...
moduleFor('Components test: {{input}} with dynamic type', class extends InputRenderingTest {
...
moduleFor(`Components test: {{input type='checkbox'}}`, class extends InputRenderingTest {
  ['@test dynamic attributes']() {
    this.render(`{{input
      type='checkbox'
      disabled=disabled
...
moduleFor(`Components test: {{input type='text'}}`, class extends InputRenderingTest {
  ['@test null values']() {
    ...
    runTask(() => {
      ...
      set(this.context, 'maxlength', null); // set(this.context, 'size', null); //NOTE: this fails with `Error: Failed to set the 'size' property on 'HTMLInputElement': The value provided is 0, which is an invalid size.` (TEST_SUITE=sauce)
...
['type="range" min="-5" max="50" value="%x"', 'type="range" value="%x" min="-5" max="50"', 'min="-5" max="50" type="range" value="%x"', 'min="-5" max="50" value="%x" type="range"', 'value="%x" min="-5" max="50" type="range"', 'value="%x" type="range" min="-5" max="50"'].forEach(attrs => {
  moduleFor(`[GH#15675] Components test: {{input ${attrs}}}`, class extends InputRenderingTest {
