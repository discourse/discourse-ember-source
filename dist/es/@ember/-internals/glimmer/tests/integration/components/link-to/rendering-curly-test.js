...
moduleFor('{{link-to}} component (rendering tests)', class extends ApplicationTestCase {
  [`@feature(ember-glimmer-angle-bracket-built-ins) throws a useful error if you invoke it wrong`](assert) {
    assert.expect(1);
    expectAssertion(() => {
  ...
  async [`@feature(!ember-glimmer-angle-bracket-built-ins) throws a useful error if you invoke it wrong`](assert) {
    assert.expect(1);
    this.addTemplate('application', `{{#link-to id='the-link'}}Index{{/link-to}}`);
    await assert.rejectsAssertion(this.visit('/'), /You must provide one or more parameters to the link-to component/);
  }
...
moduleFor('{{link-to}} component (rendering tests, without router)', class extends RenderingTestCase {
