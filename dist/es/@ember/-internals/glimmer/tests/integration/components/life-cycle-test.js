import { moduleFor, RenderingTestCase, classes, strip, runAppend, runTask } from 'internal-test-helpers';
...
class LifeCycleHooksTest extends RenderingTestCase {
  ...
  ['@test components rendered from `{{each}}` have correct life-cycle hooks to be called']() {
    ...
    this.assertHooks({
      label: 'after initial render',
      interactive: [// Sync hooks
      ...initialHooks(1), ...initialHooks(2), ...initialHooks(3), ...initialHooks(4), ...initialHooks(5), // Async hooks
    ...
    runTask(() => set(this.context, 'items', [])); // TODO: Is this correct? Should childViews be populated in non-interactive mode?
...
moduleFor('Run loop and lifecycle hooks', class extends RenderingTestCase {
...
if (!jQueryDisabled) {
  moduleFor('Run loop and lifecycle hooks - jQuery only', class extends RenderingTestCase {
