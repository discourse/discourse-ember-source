...
moduleFor('Helpers test: {{readonly}}', class extends RenderingTestCase {
  ['@test {{readonly}} of a path should work']() {
    ...
    this.assert.equal(get(this.context, 'val'), 12, 'upstream attribute is not updated'); // No U-R
  }
  ...
  '@test updating a nested path of a {{readonly}}'(assert) {
    ...
    assert.deepEqual(get(component, 'value'), {
      prop: 'initial'
    });
  ...
  ['@test {{mut}} of a {{readonly}} mutates only the middle and bottom tiers']() {
    ...
    this.assert.equal(get(middle, 'foo'), 12, "middle's local foo received the value"); // updating the mut-cell directly
    ...
    this.assert.equal(get(middle, 'foo'), 10, "middle's local foo was updated after set of context's val"); // setting as a normal property
