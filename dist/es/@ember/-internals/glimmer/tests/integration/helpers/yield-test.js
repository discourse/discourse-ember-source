...
moduleFor('Helpers test: {{yield}} helper', class extends RenderingTestCase {
  ['@test can yield to block']() {
    ...
    runTask(() => set(this.context, 'object', {
      title: 'Seattle'
    }));
    this.assertText('[In layout:] [In Block:] Seattle');
  }
  ...
  ['@test yield uses the original context']() {
    let KiwiCompComponent = Component.extend({
      boundText: 'Inner'
    });
    this.registerComponent('kiwi-comp', {
  ...
  ['@test yield should not introduce a view'](assert) {
    let ParentCompComponent = Component.extend({
