...
moduleFor('Helpers test: {{array}}', class extends RenderingTestCase {
  ...
  ['@test binds values when variables are used']() {
    this.render(strip`{{#with (array personOne) as |people|}}
              {{#each people as |personName|}}
                {{personName}}
              {{/each}}
            {{/with}}`, {
  ...
  ['@test binds multiple values when variables are used']() {
    this.render(strip`{{#with (array personOne personTwo) as |people|}}
              {{#each people as |personName|}}
                {{personName}},
              {{/each}}
            {{/with}}`, {
  ...
  ['@test array helpers can be nested']() {
    this.render(strip`{{#with (array (array personOne personTwo)) as |listOfPeople|}}
  ...
  ['@test should yield hash of an array of internal and external properties']() {
    ...
    this.render(strip`{{#foo-bar personTwo=model.personTwo as |values|}}
              {{#each values.people as |personName|}}
                {{personName}},
              {{/each}}
            {{/foo-bar}}`, {
  ...
  ['@test should render when passing as argument to a component invocation']() {
    ...
    this.render(strip`{{foo-bar people=(array "Tom" personTwo)}}`, {
      personTwo: 'Chad'
    });
