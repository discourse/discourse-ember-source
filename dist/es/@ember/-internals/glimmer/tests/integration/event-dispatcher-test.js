...
import { subscribe as instrumentationSubscribe, reset as instrumentationReset } from '@ember/instrumentation';
...
moduleFor('EventDispatcher', class extends RenderingTestCase {
...
moduleFor('EventDispatcher#setup', class extends RenderingTestCase {
  constructor() {
    super(...arguments);
...
if (EMBER_IMPROVED_INSTRUMENTATION) {
  moduleFor('EventDispatcher - Instrumentation', class extends RenderingTestCase {
    teardown() {
      super.teardown();
...
if (canDataTransfer) {
  moduleFor('EventDispatcher - Event Properties', class extends RenderingTestCase {
...
if (jQueryDisabled) {
  moduleFor('EventDispatcher#native-events', class extends RenderingTestCase {
  ...
  moduleFor('EventDispatcher#jquery-events', class extends RenderingTestCase {
    beforeEach() {
    ...
    [`@${HAS_NATIVE_PROXY && DEBUG ? 'test' : 'skip'} accessing jQuery.Event#__originalEvent does not trigger deprecations to support ember-jquery-legacy`](assert) {
      let receivedEvent;
      this.registerComponent('x-foo', {
        ComponentClass: Component.extend({
          click(event) {
