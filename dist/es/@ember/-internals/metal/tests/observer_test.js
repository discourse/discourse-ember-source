...
moduleFor('addObserver', class extends AbstractTestCase {
  ...
  async ['@test observer added via Function.prototype extensions and brace expansion should fire when property changes'](assert) {
  ...
  async ['@test observer specified via Function.prototype extensions via brace expansion should fire when dependent property changes'](assert) {
    if (!FUNCTION_PROTOTYPE_EXTENSIONS && ENV.EXTEND_PROTOTYPES.Function) {
      let obj = {
        baz: 'Initial'
      };
      ...
      get(obj, 'foo');
      set(obj, 'baz', 'Baz');
      await runLoopSettled(); // fire once for foo, once for bar
