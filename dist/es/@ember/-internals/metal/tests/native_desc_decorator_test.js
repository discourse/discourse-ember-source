...
classes.forEach(TestClass => {
  moduleFor(TestClass.module('@ember/-internals/metal/nativeDescDecorator'), class extends AbstractTestCase {
    ['@test defining a configurable property'](assert) {
      let factory = new TestClass(assert);
      factory.install('foo', nativeDescDecorator({
        configurable: true,
        value: 'bar'
      }), assert);
    ...
    ['@test combining multiple setter and getters'](assert) {
      ...
      factory.install('bar', nativeDescDecorator({
        get: function () {
          return this.__bar__;
        },
        set: function (value) {
      ...
      factory.install('fooBar', nativeDescDecorator({
        get: function () {
