...
import { computed, getCachedValueFor, defineProperty, isClassicDecorator, isComputed, get, set, isWatching, addObserver } from '..';
import { meta as metaFor } from '@ember/-internals/meta';
import { EMBER_METAL_TRACKED_PROPERTIES } from '@ember/canary-features';
import { moduleFor, AbstractTestCase, runLoopSettled } from 'internal-test-helpers';
let obj, count;
moduleFor('computed', class extends AbstractTestCase {
...
let objA, objB;
moduleFor('computed should inherit through prototype', class extends AbstractTestCase {
  beforeEach() {
    objA = {
      __foo: 'FOO'
    };
...
moduleFor('redefining computed property to normal', class extends AbstractTestCase {
  beforeEach() {
    objA = {
      __foo: 'FOO'
    };
...
moduleFor('redefining computed property to another property', class extends AbstractTestCase {
  beforeEach() {
    objA = {
      __foo: 'FOO'
    };
...
moduleFor('computed - metadata', class extends AbstractTestCase {
...
moduleFor('computed - cacheable', class extends AbstractTestCase {
  beforeEach() {
    obj = {};
    count = 0;
  ...
  ['@test setting a cached computed property passes the old value as the third argument'](assert) {
    ...
    defineProperty(obj, 'plusOne', computed('foo', {
      get() {},
...
moduleFor('computed - dependentkey', class extends AbstractTestCase {
  beforeEach() {
    obj = {
      bar: 'baz'
    };
    ...
    defineProperty(obj, 'foo', computed('bar', {
      get: getterAndSetter,
      set: getterAndSetter
    }));
  }
  ...
  ['@test circular keys should not blow up'](assert) {
    ...
    defineProperty(obj, 'bar', computed('foo', {
      get: func,
      set: func
    }));
  ...
  ['@test redefining a property should undo old dependent keys'](assert) {
    ...
    defineProperty(obj, 'foo', computed('baz', function () {
      count++;
      return 'baz ' + count;
    }));
  ...
  ['@test can watch multiple dependent keys specified declaratively via brace expansion'](assert) {
    defineProperty(obj, 'foo', computed('qux.{bar,baz}', function () {
      count++;
      return 'foo ' + count;
    }));
    assert.equal(get(obj, 'foo'), 'foo 1', 'get once');
  ...
  ['@test throws assertion if brace expansion notation has spaces']() {
    expectAssertion(function () {
      defineProperty(obj, 'roo', computed('fee.{bar, baz,bop , }', function () {
        count++;
        return 'roo ' + count;
      }));
  ...
  ['@test throws an assertion if an uncached `get` is called after object is destroyed']() {
    ...
    let message = EMBER_METAL_TRACKED_PROPERTIES ? 'Attempted to access the computed <custom-obj:here>.foo on a destroyed object, which is not allowed' : 'Cannot modify dependent keys for `foo` on `<custom-obj:here>` after it has been destroyed.';
...
if (!EMBER_METAL_TRACKED_PROPERTIES) {
  moduleFor('computed - dependentkey - watching', class extends AbstractTestCase {
    beforeEach() {
      obj = {
        bar: 'baz'
      };
...
let func;
moduleFor('computed - dependentkey with chained properties', class extends AbstractTestCase {
  beforeEach() {
    obj = {
      foo: {
        bar: {
...
moduleFor('computed - improved cp syntax', class extends AbstractTestCase {
...
moduleFor('computed edge cases', class extends AbstractTestCase {
...
moduleFor('computed - setter', class extends AbstractTestCase {
  async ['@test setting a watched computed property'](assert) {
    let obj = {
      firstName: 'Yehuda',
      lastName: 'Katz'
    };
    defineProperty(obj, 'fullName', computed('firstName', 'lastName', {
      get() {
...
moduleFor('computed - default setter', class extends AbstractTestCase {
...
moduleFor('computed - readOnly', class extends AbstractTestCase {
