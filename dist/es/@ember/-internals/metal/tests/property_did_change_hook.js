...
moduleFor('PROPERTY_DID_CHANGE', class extends AbstractTestCase {
  ['@test alias and cp'](assert) {
    ...
    assert.equal(get(obj, 'cost'), undefined); // this is how PROPERTY_DID_CHANGE will get notified
    ...
    assert.equal(get(obj, 'tax'), undefined); // this is how PROPERTY_DID_CHANGE will get notified

    assert.ok(isWatching(obj, 'child.tax'), 'alias target `child.cost` is watched after consumption'); // increments the watching count on the alias itself to 1

    assert.ok(isNaN(get(obj, 'total')), 'total is initialized'); // decrements the watching count on the alias itself to 0
