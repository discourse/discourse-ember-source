...
moduleFor('Ember.PromiseProxy - ObjectProxy', class extends AbstractTestCase {
  beforeEach() {
  ...
  ['@test fulfillment'](assert) {
    ...
    assert.equal(get(proxy, 'isSettled'), true, 'expects the proxy to indicate that it is settled');
    assert.equal(get(proxy, 'isRejected'), false, 'expects the proxy to indicate that it is not rejected');
    assert.equal(get(proxy, 'isFulfilled'), true, 'expects the proxy to indicate that it is fulfilled'); // rest of the promise semantics are tested in directly in RSVP
  }
