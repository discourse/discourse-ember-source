...
class IncludesTests extends AbstractTestCase {
  ...
  '@test includes returns true if startAt + length is still negative'() {
    ...
    this.assert.equal(obj.includes(newFixture(1), -2), false, 'should return false if not included');
  }
  ...
  '@test includes returns false if startAt out of bounds'() {
    ...
    this.assert.equal(obj.includes(newFixture(1), 2), false, 'should return false if startAt >= length');
  }
