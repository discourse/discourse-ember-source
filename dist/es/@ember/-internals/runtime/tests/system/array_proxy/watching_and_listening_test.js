...
moduleFor('ArrayProxy - watching and listening', class extends AbstractTestCase {
  ...
  [`@test regression test for https://github.com/emberjs/ember.js/issues/12475`](assert) {
    ...
    addObserver(item1c, 'id', function () {}); // The EachProxy has not yet been consumed. Only the manually added
    // observers are watching.

    assert.equal(watcherCount(item1a, 'id'), 1);
    assert.equal(watcherCount(item1b, 'id'), 1);
    assert.equal(watcherCount(item1c, 'id'), 1); // Consume the each proxy. This causes the EachProxy to add two observers
    // per item: one for "before" events and one for "after" events.

    assert.deepEqual(get(obj, 'ids'), [1, 2, 3]); // For each item, the two each proxy observers and one manual added observer
    // are watching.

    assert.equal(watcherCount(item1a, 'id'), 2);
    assert.equal(watcherCount(item1b, 'id'), 2);
    assert.equal(watcherCount(item1c, 'id'), 2); // This should be a no-op because observers do not fire if the value
    // 1. is an object and 2. is the same as the old value.

    proxy.set('content', content1);
    assert.equal(watcherCount(item1a, 'id'), 2);
    assert.equal(watcherCount(item1b, 'id'), 2);
    assert.equal(watcherCount(item1c, 'id'), 2); // This is repeated to catch the regression. It should still be a no-op.

    proxy.set('content', content1);
    assert.equal(watcherCount(item1a, 'id'), 2);
    assert.equal(watcherCount(item1b, 'id'), 2);
    assert.equal(watcherCount(item1c, 'id'), 2); // Set the content to a new array with completely different items and
    // repeat the process.
    ...
    assert.equal(watcherCount(item2c, 'id'), 2); // Ensure that the observers added by the EachProxy on all items in the
