...
class RemoveAtTests extends AbstractTestCase {
  async '@test removeAt([X], 0) => [] + notify'() {
    ...
    this.assert.equal(removeAt(obj, 0), obj, 'return self'); // flush observers
  ...
  async '@test removeAt([A,B], 1) => [A] + notify'() {
    ...
    this.assert.equal(removeAt(obj, 1), obj, 'return self'); // flush observers
  ...
  async '@test removeAt([A,B,C,D], 1,2) => [A,D] + notify'() {
    ...
    this.assert.equal(removeAt(obj, 1, 2), obj, 'return self'); // flush observers
