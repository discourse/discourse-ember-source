...
function transformPositionalLinkToIntoNamedArguments(env, node) {
    ...
    let { params, hash: { pairs }, } = node;
    let keys = pairs.map(pair => pair.key);
    if (params.length === 0) {
        assert(`You must provide one or more parameters to the \`{{link-to}}\` component. ${calculateLocationDisplay(moduleName, node.loc)}`, keys.indexOf('params') !== -1 ||
        ...
        assert(`You cannot pass positional parameters and the \`params\` argument to the \`{{link-to}}\` component at the same time. ${calculateLocationDisplay(moduleName, node.loc)}`, keys.indexOf('params') === -1);
        assert(`You cannot pass positional parameters and the \`route\` argument to the \`{{link-to}}\` component at the same time. ${calculateLocationDisplay(moduleName, node.loc)}`, keys.indexOf('route') === -1);
        assert(`You cannot pass positional parameters and the \`model\` argument to the \`{{link-to}}\` component at the same time. ${calculateLocationDisplay(moduleName, node.loc)}`, keys.indexOf('model') === -1);
        assert(`You cannot pass positional parameters and the \`models\` argument to the \`{{link-to}}\` component at the same time. ${calculateLocationDisplay(moduleName, node.loc)}`, keys.indexOf('models') === -1);
        assert(`You cannot pass positional parameters and the \`query\` argument to the \`{{link-to}}\` component at the same time. ${calculateLocationDisplay(moduleName, node.loc)}`, keys.indexOf('query') === -1);
    }
    assert(`You must provide one or more parameters to the \`{{link-to}}\` component. ${calculateLocationDisplay(moduleName, node.loc)}`, params.length > 0);
    // 1. The last argument is possibly the `query` object.
    let query = params[params.length - 1];
    if (query && isQueryParams(query)) {
        params.pop();
        assert(`The \`(query-params ...)\` helper does not take positional arguments. ${calculateLocationDisplay(moduleName, query.loc)}`, query.params.length === 0);
...
export default function transformLinkTo(env) {
    return {
