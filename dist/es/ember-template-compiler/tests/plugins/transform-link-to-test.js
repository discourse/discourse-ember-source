...
if (EMBER_GLIMMER_ANGLE_BRACKET_BUILT_INS) {
  moduleFor('ember-template-compiler: transforming inline {{link-to}} into the block form', class extends TransformTestCase {
  ...
  moduleFor('ember-template-compiler: transforming positional arguments into named arguments', class extends TransformTestCase {
    ['@test no arguments']() {
      expectAssertion(() => compile('{{#link-to}}zomg{{/link-to}}', {
        moduleName: '-top-level'
      }), /You must provide one or more parameters to the `{{link-to}}` component. \('-top-level' @ L1:C0\)/);
      expectAssertion(() => compile('{{#link-to class="wow"}}zomg{{/link-to}}', {
        moduleName: '-top-level'
      }), /You must provide one or more parameters to the `{{link-to}}` component. \('-top-level' @ L1:C0\)/); // these are ok

      compile('{{#link-to params=foo}}zomg{{/link-to}}', {
        moduleName: '-top-level'
      });
      compile('{{#link-to route=foo}}zomg{{/link-to}}', {
        moduleName: '-top-level'
      });
      compile('{{#link-to model=foo}}zomg{{/link-to}}', {
        moduleName: '-top-level'
      });
      compile('{{#link-to models=foo}}zomg{{/link-to}}', {
        moduleName: '-top-level'
      });
      compile('{{#link-to query=foo}}zomg{{/link-to}}', {
        moduleName: '-top-level'
      });
    }

    ['@test mixing positional and named arguments']() {
      expectAssertion(() => compile('{{#link-to foo params=bar}}zomg{{/link-to}}', {
        moduleName: '-top-level'
      }), /cannot pass positional parameters and the `params` argument to the `{{link-to}}` component at the same time. \('-top-level' @ L1:C0\)/);
      expectAssertion(() => compile('{{#link-to foo route=bar}}zomg{{/link-to}}', {
        moduleName: '-top-level'
      }), /cannot pass positional parameters and the `route` argument to the `{{link-to}}` component at the same time. \('-top-level' @ L1:C0\)/);
      expectAssertion(() => compile('{{#link-to foo model=bar}}zomg{{/link-to}}', {
        moduleName: '-top-level'
      }), /cannot pass positional parameters and the `model` argument to the `{{link-to}}` component at the same time. \('-top-level' @ L1:C0\)/);
      expectAssertion(() => compile('{{#link-to foo models=bar}}zomg{{/link-to}}', {
        moduleName: '-top-level'
      }), /cannot pass positional parameters and the `models` argument to the `{{link-to}}` component at the same time. \('-top-level' @ L1:C0\)/);
      expectAssertion(() => compile('{{#link-to foo query=bar}}zomg{{/link-to}}', {
        moduleName: '-top-level'
