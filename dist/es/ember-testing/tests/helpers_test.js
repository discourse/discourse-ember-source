...
import { pendingRequests, incrementPendingRequests, decrementPendingRequests, clearPendingRequests } from '../lib/test/pending_requests';
...
if (!jQueryDisabled) {
  moduleFor('ember-testing: Helper setup', class extends HelpersTestCase {
    [`@test Ember.Application#injectTestHelpers/#removeTestHelper`](assert) {
      ...
      this.application.removeTestHelpers();
      assertNoHelpers(assert, this.application);
      assert.equal(Test.Promise.prototype.LeakyMcLeakLeak, undefined, 'should NOT leak test promise extensions');
    }
  ...
  moduleFor('ember-testing: pendingRequests', class extends HelpersApplicationTestCase {
    ...
    [`@test pendingRequests is ignores ajaxComplete events from past setupForTesting calls`](assert) {
      ...
      assert.equal(pendingRequests(), 1, 'Ember.Test.pendingRequests was incremented');
      this.trigger('ajaxComplete', xhr);
      assert.equal(pendingRequests(), 1, 'Ember.Test.pendingRequests is not impressed with your unexpected complete');
    }
