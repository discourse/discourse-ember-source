...
import { FEATURES, EMBER_NATIVE_DECORATOR_SUPPORT, EMBER_METAL_TRACKED_PROPERTIES } from '@ember/canary-features';
import { confirmExport } from 'internal-test-helpers';
import { moduleFor, AbstractTestCase } from 'internal-test-helpers';
import { jQueryDisabled, jQuery } from '@ember/-internals/views';
moduleFor('ember reexports', class extends AbstractTestCase {
  ...
  ['@test Ember.EXTEND_PROTOTYPES is present (but deprecated)'](assert) {
    expectDeprecation(() => {
      assert.strictEqual(Ember.ENV.EXTEND_PROTOTYPES, Ember.EXTEND_PROTOTYPES, 'Ember.EXTEND_PROTOTYPES exists');
    }, /EXTEND_PROTOTYPES is deprecated/);
  }

  '@test Ember.FEATURES is exported'(assert) {
    for (let feature in FEATURES) {
      assert.equal(Ember.FEATURES[feature], FEATURES[feature], 'Ember.FEATURES contains ${feature} with correct value');
    }
  }
...
if (!jQueryDisabled) {
  moduleFor('ember reexports: jQuery enabled', class extends AbstractTestCase {
