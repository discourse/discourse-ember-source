...
moduleFor('Router Service - events', class extends RouterTestCase {
  ...
  '@test nested params'(assert) {
    assert.expect(30);
    let initial = true;
    this.add('route:dynamicWithChild', Route.extend({
      model(params) {
        if (initial) {
          assert.deepEqual(params, {
            dynamic_id: '123'
    ...
    this.add('route:dynamicWithChild.child', Route.extend({
      model(params) {
        assert.deepEqual(params, {
          child_id: '456'
        });
...
moduleFor('Router Service - deprecated events', class extends RouterTestCase {
...
moduleFor('Router Service: deprecated willTransition hook', class extends RouterTestCase {
  get routerOptions() {
    return {
      willTransition() {
...
moduleFor('Router Service: deprecated didTransition hook', class extends RouterTestCase {
  get routerOptions() {
    return {
      didTransition() {
        this._super(...arguments); // Overrides
